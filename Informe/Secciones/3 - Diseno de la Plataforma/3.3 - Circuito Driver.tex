...
Este es el llamado {\Medium circuito \textit{driver}} o {\Medium circuito de excitación} y debe existir uno para cada uno de los cuatro transistores del puente. Ahora debemos establecer algunos requerimientos que se tienen que cumplir o son deseables del circuito:\\
...
\begin{center}
    ...
    \end{tabular}
    \caption{Especificaciones del driver para MOSFET modelo 2ED21834-S06J de Infineon Technologies.\textsuperscript{\cite{DatasheetDriver}}}
    \label{tabla:driver}
\end{center}
\end{table}
...
Aquí se puede ver el driver, indicado por la referencia U12, al que le llegan las señales de comando PWM a sus pines HIN y /LIN para el transistor del lado alto y bajo de la columna respectivamente (al estar negada  la entrada para el transistor bajo, la señal que le llega debe estar invertida). Luego, conectado entre el pin DT y tierra se encuentra la resistencia de dead-time, que con un valor de \SI[]{0}[]{\ohm} resulta en un tiempo de \SI[]{400}[]{\nano\second}. En la salida, se conecta a los pines HO (alto) y LO (bajo) una resistencia limitadora en paralelo con un diodo par permitir la descarga de las capacitancias de los transistores, y entre los pines VB y VS se coloca el capacitor de bootstrap, dimensionado de acuerdo a la nota de aplicación \cite{Bootstrap}.\\

El resto de los componentes conforman conexiones adicionales de capacitores y resistencias que se realizaron según la recomendación que Infineon provee en la hoja de datos del driver. Además al estar del lado primario, todas su conexiones a tierra se realizan a $GND_1$.\\
