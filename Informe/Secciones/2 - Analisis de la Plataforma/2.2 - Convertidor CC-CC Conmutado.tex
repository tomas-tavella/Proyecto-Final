...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.7]{Imagenes/Proto Reductor.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.7]{Imagenes/Reductor.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.7]{Imagenes/Reductor Llave Cerrada.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.7]{Imagenes/Reductor Llave Abierta.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.7]{Imagenes/Desarrollo Forward 1.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.7]{Imagenes/Forward.pdf}
    \caption{Circuito de un convertidor forward, desarrollado a partir del circuito de un reductor.}
    \label{forward}
\end{figure}
...
Para solucionar este inconveniente, se puede diseñar un converitdor forward de dos llaves o \textit{double-ended}, que disminuye el estrés de tensión de cada llave a $V_s$.\\
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.7]{Imagenes/Forward Double-Ended.pdf}
    \caption{Circuito de un convertidor forward de doble llave o double-ended.}
    \label{forward_doubleended}
\end{figure}

Una consecuencia que existe para ambos tipos de convertidores forward (normal y double-ended), que surge de la necesidad de restablecer el flujo magnetizante del núcleo, es que al estar el ciclo de trabajo limitado a la mitad del período, los requerimientos de filtrado aumentan considerablemente. Esto requiere de la utilización de una bobina $L$ de mucha mayor inductancia, lo que aumenta su costo y tamaño, además de introducir una gran cantidad de armónicos de la frecuencia de conmutación que distorsionan la forma de onda de salida.\textsuperscript{\cite{SoftSwitchPWM}}\\

Entonces, se debe encontrar un circuito que sea capaz de obtener una tensión rectificada de secundario que supere el 50 \% de ciclo de trabajo, disminuyendo los requerimientos del filtro y la presencia de armónicos no deseados. Es posible obtener un circuito de estas características, el cual se va a desarrollar partiendo del circuito del convertidor forward de una llave de la figura \ref{forward}.\\

\paragraph{El Convertidor Push-Pull}

Una posible solución podría ser la conexión de dos convertidores forward en paralelo en el primario, que luego compartan el diodo $D_1$ y el filtro de salida LC del lado secundario. Los interruptores $Q_1$ y $Q_2$ deben operar de manera complementaria, con $Q_1$ conduciendo cuando $Q_2$ se abre y viceversa (ambos con el mismo ciclo de trabajo); así se obtiene una tensión rectificada del doble de ciclo de trabajo de de cada llave.\\
...
El circuito resultante de la figura \ref{desarrollo_pushpull}, sin embargo, se puede simplificar. Si hacemos que ambos bobinados primarios compartan su núcleo magnético; entonces, se puede agregar un diodo en antiparalelo a cada llave, de manera que cada uno de estos diodos, junto con los bobinado que tienen en serie, pueden funcionar como circuitos de restablecimiento del núcleo cuando la otra llave esta conduciendo (es decir, cuando $Q_2$ conduce, el diodo antiparalelo de $Q_1$ y su bobinado restablecen la magentización del núcleo, y viceversa).\\

Entonces, los circuitos de restablecimiento heredados del convertidor forward son redundantes, y por lo tanto se pueden remover para simplificar el circuito
